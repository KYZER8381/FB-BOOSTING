...
def main_menu():
    ...
    # Approval Key Logic
    github_raw_url = 'https://github.com/KYZER8381/FB-BOOSTING/blob/main/key.txt'  # Replace with your raw GitHub URL
    stored_key = get_stored_key()

    if stored_key:
        approval_key = stored_key
        # Removed the line that printed the stored key
    else:
...
def extract_account():
    update()  # Ensure the latest updates are pulled before running
